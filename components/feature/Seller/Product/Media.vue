<template>
  <div class="flex gap-4 flex-wrap">
    <template v-if="attachments.length < max">
      ...
      >
        ...
        <p class="text-center text-xs">
          Tambahkan {{ type === "image" ? "Foto" : "Video" }} ({{ attachments.length }}/{{ max }})
        </p>
      </div>
      <input ref="inputFileElement" type="file" :accept="accept[type]" class="hidden" @change="handleChangeFile" />
    </template>
    <div
    ...
    >
      <img v-if="type === 'image'" :src="generateImage(media)" class="w-full h-full object-cover rounded" />
      <video v-if="type === 'video'" preload="metadata">
        <source :src="`${generateImage(media)}#t=0.1`" type="video/mp4" />
      </video>
      <div class="absolute bg-black/50 inset-0 items-center justify-center hidden group-hover/photos:flex">
        <UButton icon="i-heroicons:x-mark-20-solid" variant="link" @click="handleDelete(indx)" />
      </div>
    </div>
  </div>
</template>
...
function handleChangeFile(event) {
  ...
  if (!allowedExtensions.includes(`.${fileExtension}`)) {
    alert(`Format file tidak didukung. Silakan upload file ${accept[props.type]}`);
    return;
  }
