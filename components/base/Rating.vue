<template>
  <div class="flex flex-row-reverse items-center justify-end -ml-[2px]" :class="{ 'cursor-pointer': !disabled }">
    <template v-for="star in starsLength" :key="`star-${star}`">
      <UIcon
        ...
        :class="[
          ...
          modelValue && star <= modelValue ? activeColor[color] : 'text-gray-200',
          sizeClass[size],
        ]"
        @click="onChange(star)"
      />
...
const starsLength = computed(() => Array.from({ length: props.maxStars }, (_, i) => i + 1).reverse());
